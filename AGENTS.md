# AGENTS.md — belajar-vibeCod-OpenCode

This is a blank repo (zero commits, no source files) used for practicing OpenCode ("vibe coding"). Treat it as a clean slate.

## Repo state
- **Branch:** `main` (no commits yet)
- **Remote:** `https://github.com/ShofaKhafiiy/vibeCode-OpenCode.git`
- **No tooling configured:** no package.json, no build/test/lint setup, no CI, no README.

## Working here
- Before adding code or config, ask the user what they want to build or practice.
- All files are fair game — there are no conventions to follow yet.
- If you need tooling (linter, formatter, test runner), install it and commit a config along with the first meaningful code.
- `git push` likely requires user authentication setup.

## What to expect
- This is a learning/practice repo. Expect experimentation, throwaway files, and frequent resets.
- Keep things simple. No over-engineering until the user asks for it.
